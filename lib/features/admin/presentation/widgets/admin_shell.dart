...
final class AdminShell extends StatelessWidget {
  ...
  static const List<(int, IconData, String)> _destinations = [
    ...
    (2, Icons.work_outline_rounded, AppRoutes.adminExperiences),
    (3, Icons.school_outlined, AppRoutes.adminEducations),
    (4, Icons.folder_open_outlined, AppRoutes.adminProjects),
    (5, Icons.mail_outline_rounded, AppRoutes.adminMessages),
  ];

  static String _label(AppLocalizations loc, int index) => switch (index) {
    0 => loc.shellDashboard,
    1 => loc.shellProfile,
    2 => loc.shellExperience,
    3 => loc.shellEducation,
    4 => loc.shellProjects,
    _ => loc.shellMessages,
  };

  int _selectedIndexFor(String location) {
    if (location.startsWith(AppRoutes.adminMessages)) return 5;
    if (location.startsWith(AppRoutes.adminProjects)) return 4;
    if (location.startsWith(AppRoutes.adminEducations)) return 3;
    if (location.startsWith(AppRoutes.adminExperiences)) return 2;
